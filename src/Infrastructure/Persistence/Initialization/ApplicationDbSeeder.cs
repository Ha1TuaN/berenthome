using TD.KCN.WebApi.Infrastructure.Identity;
...
internal class ApplicationDbSeeder
{
    ...
    public ApplicationDbSeeder(TDTenantInfo currentTenant, RoleManager<ApplicationRole> roleManager, UserManager<ApplicationUser> userManager, CustomSeederRunner seederRunner, ILogger<ApplicationDbSeeder> logger)
    {
    ...
    private async Task SeedAdminUserAsync()
    {
        ...
        {
            string adminUserName = $"{_currentTenant.Id.Trim()}.{TDRoles.Admin}".ToLowerInvariant();
            adminUser = new ApplicationUser
            {
                FullName = TDRoles.Admin,
        ...
        // Assign role to user
        if (!await _userManager.IsInRoleAsync(adminUser, TDRoles.Admin))
        {
            _logger.LogInformation("Assigning Admin Role to Admin User for '{tenantId}' Tenant.", _currentTenant.Id);
            await _userManager.AddToRoleAsync(adminUser, TDRoles.Admin);
        }
    }
}